    ...
    <div className="p-6 border-2 border-[#F2F2F2] rounded-2xl">
      ...
      {items.length !== 0
        ? items.map((item, index) => {
            return <SelectedCard item={item} key={index} deleteItem={() => deleteItem(index)} />;
          })
        : ""}
      ...
      <button onClick={emptyCart} className="btn rounded-full bg-linear-to-r from-[#4F39F6] to-[#9514FA] text-white w-full">
        Proceed To Checkout
      </button>
    </div>
  );
